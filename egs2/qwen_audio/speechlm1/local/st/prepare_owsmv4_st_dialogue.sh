...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    for split in ${splits}; do
        ...
        python local/st/prepare_owsmv4_st_dialogue.py \
            --data_dir ${data_dir} \
            --dump_dir ${dumpdir} \
            --dump_audio_dir ${dumpdir_audio} \
            --dump_kaldi_dir ${dumpdir_kaldi} \
            --split ${split} \
            --prompt_json st_prompt.json \
        
        cp ${dumpdir}/raw_audio_text_dialogue_${split}_st/data/dialogue.1 ${dumpdir}/raw_audio_text_dialogue_${split}_st/dialogue

        python pyscripts/utils/make_speechlm_json.py \
            --output_json ${dumpdir}/raw_audio_text_dialogue_${split}_st/data.json \
            --task audio_text_dialogue \
            --file_modality_type ${dumpdir}/raw_audio_text_dialogue_${split}_st/dialogue,dialogue,dialogue_json \
