...
#     the merged train_v* and dev_v* for feature extractions. Instead,
...
if [ ${VERSION} = "v1" ]; then
    ...
    train_sets="data/LibriSpeech/train-clean-100 \
                data/LibriSpeech/train-clean-360 \
                data/LibriSpeech/train-other-500"
    dev_sets="data/LibriSpeech/dev-clean \
              data/LibriSpeech/dev-other"
    test_sets="data/LibriSpeech/test-clean \
...
# ReazonSpeech: no train / valid /test splits. Generated by ourselves.
    ...
    train_sets="data/aidatatang/train_whisper \
                ...
                data/babel/train \
                data/openslr/train"
    dev_sets="data/aidatatang/dev_whisper \
...
train_out=data/train_librispeech
dev_out=data/dev_librispeech
test_out=data/test_librispeech
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    # Combine test
    utils/combine_data.sh --skip_fix true --extra-files "${utt_extra_files}" \
        ${test_out} ${test_sets} || exit 1;
    # NOTE(yifan): extra text files must be sorted and unique
    for f in ${utt_extra_files}; do
        check_sorted ${test_out}/${f}
    done
    utils/fix_data_dir.sh --utt_extra_files "${utt_extra_files}" ${test_out} || exit 1;
    utils/validate_data_dir.sh --no-feats --non-print ${test_out} || exit 1;
fi
