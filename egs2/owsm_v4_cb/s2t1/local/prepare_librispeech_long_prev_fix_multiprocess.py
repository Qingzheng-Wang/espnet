"""Prepare LibriSpeech data for English ASR with long previous context (multiprocess version)."""
...
def get_audio_duration_batch(audio_paths: List[str], max_workers: int = 8) -> Dict[str, float]:
    """Get audio durations for multiple files using multithreading."""
    durations = {}
    
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_path = {
            executor.submit(get_audio_duration, path): path 
            for path in audio_paths
        }
        
        # 收集结果，使用tqdm显示进度
        for future in tqdm(as_completed(future_to_path), total=len(audio_paths), desc="Getting durations"):
...
def collect_data(
    ...
    # 首先收集所有音频文件信息
    all_audio_info = []  # (audio_path, speaker_id, chapter_id, chapter_key, utt_id, text)
    
    # 读取所有speaker目录，先收集文件信息
            ...
            with open(trans_file, 'r') as f:
                for line in f:
                    ...
                    if audio_file.exists():
                        all_audio_info.append((
                            str(audio_file), speaker_id, chapter_id, chapter_key, utt_id, text
                        ))
                    else:
    ...
    # 创建utterances，使用已获取的时长信息
    for audio_path, speaker_id, chapter_id, chapter_key, utt_id, text in all_audio_info:
        duration = durations.get(audio_path, 0.0)
        if duration == 0.0:
            continue
...
def parse_args():
    parser = ArgumentParser(description="Prepare LibriSpeech data with long previous context (multiprocess version).")
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Maximum number of worker threads for audio processing (default: 8).",
    )
