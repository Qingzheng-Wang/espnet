...
# Paths
manifest_dir=./manifest_arkive
stats_dir=./exp_arkive/speechlm_stats
output_dir=./exp_arkive/train_speechlm_$(date +%Y%m%d_%H%M%S)
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    if [ -f ${stats_dir}/stats_audio_to_text_libri960.jsonl ]; then
        ...
        python3 ../../../espnet2/speechlm/bin/prepare_length_stats.py \
            ...
            --train-unregistered-specifier "audio_to_text:libri_dev:${manifest_dir}/dev/dataset.json:1.0" \
            --valid-unregistered-specifier "audio_to_text:libri_test_clean:${manifest_dir}/test_clean/dataset.json:1.0" \
            --num-workers 32 \
            --log-level INFO
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    deepspeed --num_gpus=${ngpu} \
        ...
        --stats-dir ${stats_dir} \
        --logger-type swanlab \
        --swanlab-project "speechlm" \
        --swanlab-name "speechlm_librispeech_asr" \
        --swanlab-tags baseline librispeech \
        --swanlab-mode "cloud" \
        --log-level INFO
