...
def get_parser() -> argparse.ArgumentParser:
    ...
    swanlab_group.add_argument(
        ...
        help="SwanLab logging mode: cloud (online), local (offline), or disabled",
    )
...
def main():
    ...
    if args.logger_type == "wandb":
        ...
        if rank == 0:
            ...
            # Determine mode: use cloud if available, otherwise local
            swanlab_mode = args.swanlab_mode if hasattr(args, 'swanlab_mode') else "cloud"
            
            swanlab.init(
                project=args.swanlab_project,
                experiment_name=swanlab_name,
                config=swanlab_config,
                mode=swanlab_mode,
                logdir=str(args.output_dir / "swanlab"),
            )
            logger.info(f"SwanLab initialization: project={args.swanlab_project}, name={swanlab_name}, mode={swanlab_mode}")
        else:
            # Non-rank-0 processes: SwanLab doesn't need explicit disable like wandb
            logger.info("SwanLab disabled on non-rank-0 process")
