...
def build_parallel_hf_class(model_hf_tag):
    ...
    class ParallelLLM(architecture):
        ...
        def from_pretrained(
            ...
            **kwargs,
            ...
            with torch.no_grad():
                ...
                new_embed_tokens.weight[0] = 0.0 # 0th token is <|pad|>, set the <|pad|>'s embedding to 0
            ...
            # pytorch nn module，是可以__setattr__, 添加新的attribute的
            model.num_stream = max(possible_num_stream)
            model.stream_emb = nn.Embedding(model.num_stream, embed_dim) # used for add on the output, to make predictin of different streams different
            ...
            for io_name, intervals in vocab_intervals.items():
                ...
                # avoid large vocabulary size, which will cause OOM
                # like, 100k size vocabulary, split into 8 13k loss intervals
        ...
        def forward(self, **kwargs):
            ...
            stream_emb = self.stream_emb.weight.tile(1, 1, 1, 1) # [num_stream, hidden_dim] -> [batch, seq_len, num_stream, hidden_dim]
        ...
        def _loss(self, hidden_states, input_ids, loss_mask):
            ...
            # The first stream might be special/text/ssl tokens, for simplicity
            # we use all lm_head weights, rather than use a interval for weights
            # we use interval for weights later in stream 1+
