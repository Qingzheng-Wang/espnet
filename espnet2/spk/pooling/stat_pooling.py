...
class StatsPooling(AbsPooling):
    ...
    def forward(
        ...
        if feat_lengths is not None:
            ...
            st = torch.sqrt(variance.clamp(
                min=torch.finfo(variance.dtype).eps, max=1e4
            )) # add max clamp to prevent gradient explosion
        else:
            mu = torch.mean(x, dim=-1)
            variance = torch.var(x, dim=-1, unbiased=False)
            st = torch.sqrt(variance.clamp(min=torch.finfo(variance.dtype).eps, max=1e4))

        x = torch.cat((mu, st), dim=1)

        return x

class StatsPoolingOrigin(AbsPooling):
    """Aggregates frame-level features to single utterance-level feature.
    ...
    def forward(
        ...
        if feat_lengths is not None:
            ...
            variance = torch.stack(
                [
                    torch.var(x[i, :, : int(l.item())], dim=-1, unbiased=False)
                    for i, l in enumerate(feat_lengths)
                ],
                dim=0,
            )  # unbiased=False (divided by N rather than N - 1)
