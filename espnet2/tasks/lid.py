...
class LIDTask(AbsTask):
    ...
    def build_model(cls, args: argparse.Namespace) -> ESPnetLIDModel:
        ...
        if args.model_conf.get("independent_module", False):
            conditioning_layers = args.model_conf.get(
                "conditioning_layers", None
            )
            if conditioning_layers is None: 
                # None means the design with lid and lang2vec condition 
                # separately.
                lang2vec_conditioning_layers = args.model_conf.get(
                    "lang2vec_conditioning_layers", None
                )
                lid_conditioning_layers = args.model_conf.get(
                    "lid_conditioning_layers", None
                )
                if (
                    lang2vec_conditioning_layers is None 
                    and lid_conditioning_layers is None
                ):
                    conditioning_layers = []
                else:
                    conditioning_layers = sorted(
                        set(
                            (lang2vec_conditioning_layers or [])
                            + (lid_conditioning_layers or [])
                        )
                    )

            # NOTE(qingzheng): In the LIDTask, we only define the conditioning 
            # modules that both lang2vec and lid will use. The specific 
            # conditioning layers, or lang2vec head, or aamsoftmax weight that 
            # specifically for lang2vec or lid will be defined in the 
            # espnet_model_upstream_condition. This is just a design choice.
            encoder_condition = nn.ModuleDict()
            pooling_condition = nn.ModuleDict()
            projector_condition = nn.ModuleDict()

            for layer_idx in conditioning_layers:
        ...
        if (
            args.model_conf.get("independent_module", False) and 
