...
class ESPnetLIDUpstreamLang2VecConditionModel(ESPnetLIDModel):
    """ESPnet LID model
    ...
    def __init__(
        ...
        apply_intermediate_loss: bool = False,
        ...
        self.frontend.upstream.upstream.model.encoder.conditioning_proj = torch.nn.Linear(
            self.loss.lang2vec_dim,
            self.frontend.upstream.upstream.model.encoder.config.hidden_size,
        )
        self.frontend.upstream.upstream.model.encoder.conditioning_layers = conditioning_layers
        self.frontend.upstream.upstream.model.encoder.frozen_ecapa = frozen_ecapa

        self.apply_intermediate_loss = apply_intermediate_loss
    ...
    def forward(
        ...
        **kwargs,
        ...
        feats, feat_lengths, intermediate_lang2vec_preds = self.extract_feats(speech, speech_lengths)
        ...
        if lang2vecs is not None:
            ...
            if (
                ...
                self.apply_intermediate_loss and 
                self.frontend.upstream.upstream.model.encoder.conditioning_layers is not None
            ):
                import logging
                logging.info(f"+++++++++++++++++++compute the intermediate loss")
    ...
    def extract_feats(
        ...
        if self.frontend is not None:
            feats, feat_lengths, intermediate_lang2vec_preds = self.frontend(speech, speech_lengths)
        else:
            feats = speech
            feat_lengths = None
